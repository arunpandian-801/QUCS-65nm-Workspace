...
<Properties>
  <View=-799,-291,2194,1402,0.864534,573,81>
  <Grid=10,10,1>
  <DataSet=7_FFT_trial.dat>
  <DataDisplay=7_FFT_trial.dpl>
  <OpenDisplay=0>
  <Script=7_FFT_trial.m>
  <RunScript=0>
  <showFrame=0>
  <FrameText0=Title>
  <FrameText1=Drawn By:>
...
<Components>
  ...
  <.TR TR1 0 30 30 0 52 0 0 "lin" 1 "0" 1 "5 ns" 1 "1000" 0 "Trapezoidal" 0 "2" 0 "1 ns" 0 "1e-16" 0 "150" 0 "0.001" 0 "1 pA" 0 "1 uV" 0 "26.85" 0 "1e-3" 0 "1e-6" 0 "1" 0 "CroutLU" 0 "no" 0 "yes" 0 "0" 0>
  <.CUSTOMSIM CUSTOM1 1 40 200 0 32 0 0 "\ntran 5.00501e-12 5e-09 0\nwrite custom#tran1#.plot v(Vout)\n\n* Make a copy of Vout\nlet tempVout = v(Vout)\n\nlinearize tempVout\n\n* Might want to set a window. Default: Hamming\n* Include if you want\n*set specwindow = ''blackman''\n\nfft tempVout\nwrite custom#fft1#.plot tempVout \n" 1 "" 0 "custom#tran1#.plot;custom#fft1#.plot" 0>
  <.FFT FFT1 0 170 30 0 52 0 0 "10GHz" 1 "10MHz" 1 "hanning" 1 "2" 0 "0" 0 "yes" 0>
</Components>
<Wires>
...
<Diagrams>
  <Rect 970 190 240 160 3 #c0c0c0 1 00 1 -1 0.5 1 1 -1 1 1 1 -1 1 1 315 0 225 1 0 0 "" "" "">
	<"ngspice/tran1.tran.v(vout)" #0000ff 2 3 0 0 0>
  </Rect>
  <Rect 460 707 933 457 3 #c0c0c0 1 10 0 1 2e+10 1e+11 1 -0.0618637 0.1 0.7 1 -1 0.2 1 315 0 225 1 1 0 "" "" "">
	<"ngspice/fft1.ac.tempvout" #ff0000 2 3 0 0 0>
	  <Mkr 9.76562e+08 322 -251 3 0 0>
  </Rect>
</Diagrams>
<Paintings>
  ...
  <Text 390 800 12 #000000 0 "The custom script is preferred\nover actual simulation controllers\ndue to understanding and\naccuracy.\n~\nI can't wrap my head around\nthe controller block.\nAs to how exactly it is implemented\n~\nSee 7-FFT-trial-2\nfor the other one in action!">
  <Text 480 760 18 #000000 0 "NOTE">
  <Rectangle 380 760 280 270 #000000 2 1 #c0c0c0 1 0>
  <Line 380 790 280 0 #000000 2 1>
  <Text 900 860 18 #000000 0 "NOTE">
  <Text 720 900 12 #000000 0 "Notice that the fundamental frequency is reported as 976 MHz.\nThis could be a reason on FFT Accuracy or not enough step in\ntransient response.\n~\nOn eye-balling, we can say, that it is roughly 1 GHz">
  ...
  <Arrow 1250 910 -30 -170 20 8 #000000 2 1 0>
</Paintings>
